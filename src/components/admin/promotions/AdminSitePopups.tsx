...
const AdminSitePopups = () => {
  ...
  const toggleActive = useMutation({
    mutationFn: async ({ id, is_active }: { id: string; is_active: boolean }) => {
      const { error } = await (supabase.from("site_popups" as any).update({ is_active }).eq("id", id) as any);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-site-popups"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await (supabase.from("site_popups" as any).delete().eq("id", id) as any);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["admin-site-popups"] });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Manage the ad banner that flashes to users when they visit the site.</p>
        </div>
      ...
      {showForm && (
        <div className="card-brutal p-6 border-2 border-foreground bg-secondary/10">
          ...
          <div className="space-y-4 max-w-xl">
            ...
            <div>
              <label className="font-heading text-xs uppercase tracking-wide mb-1 block">Redirect URL (Optional)</label>
              <input 
                type="text" 
            ...
            <div className="flex items-center gap-3 py-2">
              <input 
                ...
                className="w-4 h-4" 
              />
              <label htmlFor="is_active" className="text-sm font-medium">Set as Active immediately</label>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button onClick={() => setShowForm(false)} className="btn-brutal-secondary">Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isLoading ? (
        <div className="flex gap-4"><div className="h-48 w-64 skeleton-brutal" /></div>
      ) : popups && popups.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {popups.map((popup) => (
            <div key={popup.id} className={`card-brutal flex flex-col ${!popup.is_active ? 'opacity-70' : 'ring-2 ring-foreground'} overflow-hidden`}>
              <div className="h-48 bg-muted relative">
                <img src={popup.image_url} alt="Popup" className="w-full h-full object-cover" />
                <div className="absolute top-2 right-2 flex gap-1">
                  <span className={`px-2 py-1 text-[10px] uppercase font-bold ${popup.is_active ? 'bg-success text-success-foreground' : 'bg-muted text-muted-foreground'}`}>
              ...
              <div className="p-4 flex-1 flex flex-col bg-background">
                {popup.redirect_url ? (
                  ...
                  <p className="text-xs text-muted-foreground mb-4 italic">No redirect configured</p>
                )}
                ...
                <div className="mt-auto flex items-center justify-between gap-2 border-t border-border-subtle pt-4">
                  ...
                  <button 
                    onClick={() => { if(confirm("Delete this popup forever?")) deleteMutation.mutate(popup.id); }}
        ...
        <div className="border-2 border-dashed border-border-subtle p-12 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-muted-foreground text-sm max-w-md">Create your first global site popup ad to display to visiting users.</p>
        </div>
      )}
    </div>
  );
