...
const LuckyPromoPopup = () => {
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      const { data: rawData, error } = await supabase.rpc("claim_lucky_promo", {
        _user_id: user.id,
      });
        ...
        {!claimed ? (
          <>
            ...
            <button
              onClick={handleClaim}
              disabled={loading}
              className="btn-brutal w-full text-lg flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
          ...
          <>
            ...
            <div className="bg-secondary p-4 mb-4">
              {reward?.type === "free_shipping" ? (
                <>
                  ...
                  <p className="text-sm text-muted-foreground">₱{reward.value} off shipping fees</p>
                </>
              ) : (
                <>
            ...
            <button onClick={() => setIsVisible(false)} className="btn-brutal-secondary w-full">
              Continue Shopping
            </button>
          </>
        )}
