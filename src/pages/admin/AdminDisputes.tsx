...
const AdminDisputes = () => {
  ...
  return (
    <div className="p-4 md:p-8">
      ...
      <p className="text-muted-foreground mb-6 text-sm md:text-base">Review and resolve customer disputes ({filtered.length} total)</p>

      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
          <input type="text" placeholder="Search by reason, customer, vendor..." value={search} onChange={(e) => setSearch(e.target.value)} className="input-brutal w-full pl-10" />
        </div>
                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <span className={`px-2 py-0.5 text-xs uppercase ${statusColors[dispute.status] || "bg-muted"}`}>
                        {statusLabels[dispute.status] || dispute.status}
                      </span>
                      <span className="text-xs text-muted-foreground">{format(new Date(dispute.created_at), "PPp")}</span>
                    </div>
                    ...
                    {dispute.evidence_urls?.length > 0 && (
                      <div className="flex gap-2 mt-3">
                        {dispute.evidence_urls.map((url: string, i: number) => (
                          <a key={i} href={url} target="_blank" rel="noopener noreferrer" className="w-16 h-16 bg-muted overflow-hidden block">
                    ...
                    {dispute.resolution_notes && (
                      <div className="mt-3 p-3 bg-secondary text-sm">
                        ...
                        {dispute.refund_amount && <span className="ml-2">({formatPrice(dispute.refund_amount)})</span>}
                      </div>
                    )}
                  </div>

                  {/* Actions */}
                  {(dispute.status === "pending" || dispute.status === "under_review" || dispute.status === "escalated") && (
                    <div className="flex flex-wrap gap-2 md:flex-col md:w-40 flex-shrink-0">
                      {dispute.status === "pending" && (
                        <button onClick={() => markUnderReview(dispute.id)} className="btn-brutal text-xs w-full">
                          <Eye className="w-3 h-3 mr-1 inline" /> Review
                        </button>
                      )}
                      <button onClick={() => { setSelectedDispute(dispute); setRefundAmount(String(dispute.vendor_order?.subtotal || "")); }} className="btn-brutal text-xs w-full bg-success text-success-foreground">
                        <CheckCircle className="w-3 h-3 mr-1 inline" /> Resolve
                      </button>
                      <button onClick={() => resolveDispute(dispute.id, "resolved_dismissed")} className="btn-brutal text-xs w-full bg-muted text-muted-foreground">
      ...
      {/* Resolution Modal */}
      {selectedDispute && (
        <div className="fixed inset-0 bg-foreground/50 z-50 flex items-center justify-center p-4" onClick={() => setSelectedDispute(null)}>
          <div className="bg-background card-brutal p-6 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea value={resolutionNotes} onChange={(e) => setResolutionNotes(e.target.value)} className="input-brutal w-full h-24" placeholder="Describe the resolution..." />
              </div>
              <div>
                <label className="text-sm font-medium block mb-1">Refund Amount (optional)</label>
                <input type="number" value={refundAmount} onChange={(e) => setRefundAmount(e.target.value)} className="input-brutal w-full" placeholder="0.00" />
              </div>
              <div className="flex gap-3">
                <button disabled={resolving} onClick={() => resolveDispute(selectedDispute.id, "resolved_refund")} className="btn-brutal flex-1 bg-success text-success-foreground text-sm">
                  {resolving ? "..." : "Refund"}
                </button>
                <button disabled={resolving} onClick={() => resolveDispute(selectedDispute.id, "resolved_replacement")} className="btn-brutal flex-1 text-sm">
                  {resolving ? "..." : "Replace"}
                </button>
                <button onClick={() => setSelectedDispute(null)} className="btn-brutal flex-1 bg-muted text-muted-foreground text-sm">Cancel</button>
              </div>
            </div>
          </div>
        </div>
