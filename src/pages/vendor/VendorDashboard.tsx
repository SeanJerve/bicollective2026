...
const VendorDashboard = () => {
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [totalProducts, activeProducts, ordersData, promoStats, totalReviews, lowStockData] =
        await Promise.all([
          supabase
            .from("products")
          ...
          supabase
            ...
            .select("discount_id, discounts:discounts(is_active)")
            .eq("brand_id", brandId) as any,
          supabase.from("reviews").select("rating").eq("brand_id", brandId),
          supabase
            .from("product_variants")
            .select("stock_quantity, product:products!inner(brand_id)")
            .eq("product.brand_id", brandId)
            .lt("stock_quantity", 5) as any,
        ]);
    ...
    <div className="p-4 md:p-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 mb-6 md:mb-8">
        ...
        <div className="card-brutal p-4 md:p-6">
          <div className="flex items-center gap-3 md:gap-4">
            <div
              className={`w-10 h-10 md:w-12 md:h-12 flex items-center justify-center flex-shrink-0 ${s.lowStockProducts > 0 ? "bg-destructive/20" : "bg-secondary"}`}
            >
              <Package
                className={`w-5 h-5 md:w-6 md:h-6 ${s.lowStockProducts > 0 ? "text-destructive" : ""}`}
              />
            </div>
            <div className="min-w-0">
            ...
            {(recentOrders || []).map((order: any) => (
              <div key={order.id} className="p-6 flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <span
                    className={`inline-block px-2 py-1 text-xs uppercase ${getStatusColor(order.status)}`}
                  >
