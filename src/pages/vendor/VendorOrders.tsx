...
import { CheckCircle, XCircle, Truck, Package, Loader2, HandMetal, MapPin, AlertCircle, ChevronDown, ChevronUp, Image, ExternalLink, X } from "lucide-react";
    ...
    <div className="mt-4 md:mt-6 border-2 border-foreground p-3 md:p-4 bg-secondary">
      ...
      >
        ...
        <div className="absolute inset-0 bg-foreground/0 group-hover:bg-foreground/10 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100">
          <span className="bg-background px-3 py-1.5 border border-foreground font-heading text-[10px] uppercase shadow-brutal">Click to focus</span>
        </div>
      </div>
      ...
      {viewingImage && (
        <div className="fixed inset-0 z-[100] bg-background/95 backdrop-blur-sm p-4 md:p-12 flex flex-col animate-in fade-in zoom-in duration-200">
          ...
          <div className="flex-1 min-h-0 border-4 border-foreground bg-secondary flex items-center justify-center relative shadow-brutal-lg">
            <img src={viewingImage} alt="Full Proof" className="max-w-full max-h-full object-contain" />
          </div>
          <div className="mt-6 flex justify-center">
            <button 
              onClick={() => setViewingImage(null)}
              className="btn-brutal px-12"
            >
              CLOSE PREVIEW
            </button>
          </div>
...
const VendorOrders = () => {
  ...
  const paymentIds = useMemo(() => {
    return Array.from(new Set(Object.values(orderPayments).flat().map((p: any) => p.id))) as string[];
  }, [orderPayments]);
  ...
  const { data: paymentVerifications = {} } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase.from("payment_verifications").select("*").in("payment_id", paymentIds);
      if (error) throw error;
      const map: any = {};
      (data || []).forEach((v: any) => {
  ...
  const { data: vendorOrderItems = {} } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase.from("order_items").select("*").in("vendor_order_id", vendorOrderIds);
      if (error) throw error;
      const map: any = {};
  ...
  const addressIds = useMemo(() => {
    return Array.from(new Set(Object.values(parentOrders).map((o: any) => o.shipping_address_id).filter(Boolean))) as string[];
  }, [parentOrders]);
          ...
          {filteredOrders.map((order) => {
            ...
            <div key={order.id} className="card-brutal">
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2">
                      <p className="font-heading uppercase text-sm md:text-base">
                        <span className="font-heading uppercase">Shipping Address:</span> {order.order?.shipping_name || "Customer"}: {orderAddresses[order.order?.shipping_address_id] ? (
                    ...
                    {order.order?.payments?.[0] && (
                      <>
                        <p className="text-xs text-muted-foreground mt-1">Phone: {orderAddresses[order.order?.shipping_address_id]?.phone || order.order?.shipping_phone || 'N/A'}</p>
                  ...
                  <div className="flex sm:flex-col items-center sm:items-end gap-2 sm:gap-0">
                    ...
                    {order.order?.payments?.[0] && (
                      <span className={`inline-block px-2 py-0.5 mt-1 text-[10px] font-bold border border-foreground ${
                        (order.order.payments[0].status === "verified" || ["shipped", "for_delivery", "delivered", "confirmed"].includes(order.status)) 
                          ? "bg-success" 
                          : "bg-warning"
                      }`}>
                        {(order.order.payments[0].status === "verified" || ["shipped", "for_delivery", "delivered", "confirmed"].includes(order.status)) 
                    ...
                    <p className="font-heading text-base md:text-lg sm:mt-2 text-right">
                      {formatPrice(Number(order.subtotal) + Number(order.shipping_fee || 0) - Number(order.discount_amount || 0))}
                    </p>
                    <p className="text-[10px] text-muted-foreground uppercase font-heading text-right">Vendor Total</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 md:p-6 animate-fade-in">
                ...
                {(order.order?.shipping_address_id || order.order?.shipping_address) && (
                  <div className="mb-4 md:mb-6 p-3 md:p-4 bg-secondary">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <h4 className="font-heading text-xs md:text-sm uppercase mb-2">Shipping Information</h4>
                        <p className="text-xs md:text-sm">
                          {orderAddresses[order.order?.shipping_address_id] ? (
                            <span className="font-medium text-foreground">
                              {orderAddresses[order.order.shipping_address_id].street}, {orderAddresses[order.order.shipping_address_id].barangay}, {orderAddresses[order.order.shipping_address_id].city}, {orderAddresses[order.order.shipping_address_id].province} {orderAddresses[order.order.shipping_address_id].zip_code}
                            </span>
                          ) : (
                        ...
                        <p className="text-xs md:text-sm text-muted-foreground mt-1">
                          Recipient: {orderAddresses[order.order?.shipping_address_id]?.full_name || order.order?.shipping_name} | Phone: {orderAddresses[order.order?.shipping_address_id]?.phone || order.order?.shipping_phone}
                        </p>
                      </div>
                      <div>
                        <h4 className="font-heading text-xs md:text-sm uppercase mb-2">Payment Details</h4>
                        <div className="space-y-1 text-xs md:text-sm">
                          ...
                          <div className="flex justify-between pt-1 border-t border-border-subtle font-bold">
                            <span className="uppercase text-[10px] md:text-xs tracking-wider">Your Total:</span>
                            <span>{formatPrice(Number(order.subtotal) + Number(order.shipping_fee || 0) - Number(order.discount_amount || 0))}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                {(() => {
                  const verifications = Object.values(order.order?.payments || []).flatMap((p: any) => p.payment_verifications || []).map((v: any) => v.proof_image_url).filter(Boolean);
                    ...
                    return (
                      <div className="mt-4 p-3 border-2 border-warning bg-warning/5 animate-pulse">
                        ...
                        <p className="text-[10px] text-muted-foreground mt-1">
                          The system hasn't found an image record in the database for this order yet.
                        </p>
                      </div>
                    );
                  }
