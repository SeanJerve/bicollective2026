...
import { Tag, Plus, Percent, DollarSign, Truck, Power, Trash2, Edit2, ChevronDown, ChevronUp, BarChart3 } from "lucide-react";
...
const defaultForm: VendorPromoForm = {
  ...
  type: "percentage_discount", discount_value: 10,
  min_order_amount: 0, max_discount_amount: null,
  max_uses: null, max_uses_per_user: 1,
...
const VendorPromotions = () => {
  ...
  const { data: brand } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("brands").select("*").eq("owner_id", user!.id).single();
  ...
  const { data: products } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("products").select("id, name").eq("brand_id", brand!.id).eq("is_active", true);
      if (error) throw error;
      return data;
    },
    enabled: !!brand,
  ...
  const handleEdit = (promo: any) => {
    setForm({
      ...
      max_uses: promo.max_uses, max_uses_per_user: promo.max_uses_per_user || 1,
    ...
    <div className="p-4 md:p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Create promo codes and discounts for your products</p>
        </div>
        <button onClick={() => { setShowForm(!showForm); setEditId(null); setForm(defaultForm); }} className="btn-brutal">
      ...
      {showForm && (
        <div className="card-brutal p-6 mb-8">
          <h2 className="font-heading text-xl uppercase mb-4">{editId ? "Edit Promotion" : "Create Promotion"}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="font-heading text-xs uppercase tracking-wide mb-1 block">Name</label>
              <input className="input-brutal" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Weekend Flash Sale" />
            </div>
            <div>
              <label className="font-heading text-xs uppercase tracking-wide mb-1 block">Promo Code</label>
              <input className="input-brutal font-mono" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase() })} placeholder="FLASH20" />
            </div>
            <div>
            ...
            <div>
              <label className="font-heading text-xs uppercase tracking-wide mb-1 block">Usage Limit</label>
            ...
            <div className="md:col-span-2">
              ...
              <textarea className="input-brutal min-h-[60px]" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />
            </div>
          </div>
      ...
      {isLoading ? (
        <div className="space-y-4">{[1, 2, 3].map((i) => <div key={i} className="h-20 skeleton-brutal" />)}</div>
                  ...
                  <div className="px-4 pb-4 border-t border-border-subtle pt-3">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4 text-sm">
                      ...
                      <div>
                        ...
                        {format(new Date(promo.starts_at), "MMM d")} - {format(new Date(promo.ends_at), "MMM d")}
                      </div>
                      <div>
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => handleEdit(promo)} className="btn-brutal-secondary text-xs px-3 py-1"><Edit2 className="w-3 h-3 mr-1" />Edit</button>
                      <button onClick={() => toggleActive.mutate({ id: promo.id, is_active: !promo.is_active })} className="btn-brutal-secondary text-xs px-3 py-1">
                        <Power className="w-3 h-3 mr-1" />{promo.is_active ? "Deactivate" : "Activate"}
                      </button>
                      <button onClick={() => { if (confirm("Delete?")) deleteMutation.mutate(promo.id); }} className="btn-brutal-secondary text-xs px-3 py-1 text-destructive">
        ...
        <div className="card-brutal p-12 text-center">
          ...
          <p className="text-muted-foreground mb-4">Create promo codes and discounts for your products</p>
          <button onClick={() => setShowForm(true)} className="btn-brutal"><Plus className="w-4 h-4 mr-2" />Create Promotion</button>
        </div>
      )}
    </div>
  );
